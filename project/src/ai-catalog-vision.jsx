...
function Detail({it,persona,onBack}){
...
{persona==="User"&&<div style={{position:"relative"}} onMouseEnter={()=>setTipOpen(true)} onMouseLeave={()=>setTipOpen(false)} onFocus={()=>setTipOpen(true)} onBlur={()=>setTipOpen(false)}>
<button disabled style={{height:32,padding:"0 12px",borderRadius:4,border:`1px solid ${t.n100}`,background:t.n50,color:t.n300,fontWeight:600,fontSize:"14px",fontFamily:ff,cursor:"not-allowed"}}>{label}</button>
{tipOpen&&<div role="tooltip" style={{position:"absolute",top:"calc(100% + 6px)",right:0,width:260,background:t.n900,color:t.n0,padding:"8px 10px",borderRadius:4,fontSize:"12px",lineHeight:"16px",zIndex:20,boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}>You need the Maintainer role in frontend-app to {label.toLowerCase()} this. Ask a Maintainer on the project.</div>}
</div>}
...
<div style={{border:`1px solid ${t.n100}`,borderRadius:4,overflow:"hidden"}}>
<div style={{background:t.n50,padding:"8px 16px",borderBottom:`1px solid ${t.n100}`}}><span style={{fontSize:"14px",fontWeight:600,color:t.n950}}>{it.tp==="skill"?"Source":"Tools & system prompt"}</span></div>
<div style={{padding:16}}>
{it.tp==="skill"?<>
...
<div style={{marginBottom:16}}>
<div style={{fontSize:"14px",fontWeight:600,color:t.n950,marginBottom:8}}>Source</div>
...
<div style={{marginBottom:16}}>
<div style={{fontSize:"14px",fontWeight:600,color:t.n950,marginBottom:8}}>Bundled files</div>
<div style={{fontSize:"13px",color:t.n500,marginBottom:6,lineHeight:"18px"}}>Additional references the agent can load on demand.</div>
<div style={{border:`1px solid ${t.n100}`,borderRadius:4,overflow:"hidden"}}>
{[{n:"SKILL.md",d:"Skill definition (always loaded)",sz:"2.4 KB"},{n:"references/checklist.md",d:"Review checklist reference",sz:"8.1 KB"},{n:"references/examples.md",d:"Worked examples",sz:"12 KB"}].map((f,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",borderBottom:i<2?`1px solid ${t.n100}`:"none"}}><div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}><Ic name="snippet" size={12} color={t.n500}/><code style={{fontFamily:"monospace",fontSize:"13px",color:t.n950}}>{f.n}</code><span style={{fontSize:"12px",color:t.n400}}>{f.d}</span></div><span style={{fontSize:"12px",color:t.n400,flexShrink:0}}>{f.sz}</span></div>)}
</div>
</div>
...
{sel?<Detail it={sel} persona={persona} onBack={()=>{setSel(null)}}/>:(
<>
...
{vm==="list"&&<div style={{borderTop:`1px solid ${t.n100}`}}>{filtered.map((it,i)=><Row key={it.id} it={it} onClick={setSel} last={i===filtered.length-1}/>)}</div>}
{vm==="grid"&&<div style={{display:"grid",gridTemplateColumns:"repeat(3, minmax(0, 1fr))",gap:12}}>{filtered.map(it=><Card key={it.id} it={it} onClick={setSel}/>)}</div>}
